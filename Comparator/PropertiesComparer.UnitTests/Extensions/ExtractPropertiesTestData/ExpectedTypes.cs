using System;
namespace PropertiesComparer.UnitTests.Extensions.ExtractPropertiesTestData
{
    public class ExpectedEqutable : IEquatable<ExpectedEqutable>
    {
        public bool Equals(ExpectedEqutable other)
        {
            throw new NotImplementedException();
        }
    }

    public class ExpectedTypes
    {
        public bool T1 { get; set; }
        public byte T2 { get; set; }
        public sbyte T3 { get; set; }
        public char T4 { get; set; }
        public decimal T5 { get; set; }
        public double T6 { get; set; }
        public float T7 { get; set; }
        public int T8 { get; set; }
        public uint T9 { get; set; }
        public long T10 { get; set; }
        public ulong T11 { get; set; }
        public short T12 { get; set; }
        public ushort T13 { get; set; }
        public string T14 { get; set; }
        public decimal T15 { get; set; }
        public Guid T16 { get; set; }
        public ExpectedEqutable T17 { get; set; }
    }
}
